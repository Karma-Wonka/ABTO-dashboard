...
- **Authentication**: NextAuth (Auth.js v5), Credentials provider, bcrypt + Postgres
- **Database**: Vercel Postgres via `@vercel/postgres` (`src/lib/db.ts`)
...
- NextAuth (Auth.js v5) with a Credentials provider (email + password) — `src/lib/auth.ts`
- Passwords bcrypt-hashed into the `auth_users` Postgres table — `src/constants/auth-users.ts`
- JWT sessions; role (`admin`/`member`) embedded via the jwt/session callbacks
- Server-side guards in `src/lib/rbac.ts` (`requireRole`, `requireSelfOrAdmin`); client-side `useRole()` hook is UX-only
- Client-side RBAC for navigation visibility (`src/hooks/use-nav.ts`)
...
│   │   ├── members/       # ABTO member directory (self-service RBAC)
│   │   ├── events/        # ABTO events (admin-only writes)
│   │   ├── news/          # ABTO news (admin-only writes)
│   │   ├── documents/     # ABTO downloads/publications (admin-only writes)
│   │   ├── notifications/ # Notifications page
│   │   ├── users/         # User management demo
│   │   └── profile/       # User profile
│   ├── api/               # API routes — auth, members, events, news, documents, users
...
│   ├── auth/              # NextAuth Credentials sign-in/sign-up
│   ├── overview/          # Dashboard analytics
│   ├── members/           # ABTO member directory (React Query + nuqs)
│   │   ├── api/
...
│   │   └── schemas/       # Zod schemas
│   ├── events/            # ABTO events — same api/ pattern
│   ├── news/              # ABTO news — same api/ pattern
│   ├── documents/         # ABTO downloads/publications — same api/ pattern
...
│   ├── rbac.ts             # Server-side role guards
│   └── ...
│
...
/docs                      # Documentation
│   ├── forms.md           # Form system (TanStack Form + Zod) documentation
...
A single `scripts/cleanup.js` file handles removal of optional features. Clerk auth, the kanban board, chat, and the Forms/React Query/Icons example pages have already been removed from this ABTO fork (Clerk was replaced by NextAuth; the demo pages were deleted outright, not via this script) — the remaining modules below are still available:

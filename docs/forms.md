...
├── schemas/
│   └── entity.ts              ← Zod schema + inferred FormValues type
├── constants/
│   └── entity-options.ts      ← Select options, enums, static data
├── components/
│   ├── entity-form.tsx         ← Form UI (imports schema + options)
│   └── entity-form-fields.tsx  ← Optional: sections for large forms
```
...
Real forms in this codebase, all built on the patterns above:

| Form                         | File                                                                                         | Patterns                                    |
| ---------------------------- | --------------------------------------------------------------------------------------------- | -------------------------------------------- |
| Member listing               | `src/features/members/components/member-form.tsx`                                            | Pattern 1, split schema, self-service RBAC   |
| Event                        | `src/features/events/components/event-form.tsx`                                              | Pattern 1, split schema, onBlur validators   |
| News                         | `src/features/news/components/news-form.tsx`                                                  | Pattern 1, split schema                      |
| Document/Download            | `src/features/documents/components/document-form.tsx`                                        | Pattern 1, split schema, file metadata       |
| Credentials sign-in/sign-up  | `src/features/auth/components/credentials-sign-in-form.tsx`, `credentials-sign-up-form.tsx`  | Pattern 1, minimal                           |
